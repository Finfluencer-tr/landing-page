        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-2xl max-h-[90vh] bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl flex flex-col overflow-hidden"
                >
                    {/* Header */}
                    <div className="flex items-center justify-between p-6 border-b border-slate-800">
                        <div>
                            <h2 className="text-2xl font-bold text-white">{t.leaderboard.comments}</h2>
                                    ...
                                    <div className="flex items-start justify-between gap-4">
                                        ...
                                        {user && user.id === comment.user.id && (
                                            <button
                                                ...
                                                className="p-2 rounded-lg hover:bg-rose-500/20 text-slate-400 hover:text-rose-400 transition-colors flex-shrink-0"
                    ...
                    {user && token && (
                        <div className="p-6 border-t border-slate-800 bg-slate-900/50">
                            {error && comments.length > 0 && (
                                <div className="mb-3 p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg text-rose-400 text-sm">
                                    {error}
                                </div>
                            )}
                            <div className="flex gap-3">
                                <div className="flex-1">
                                    <textarea
                                        ...
                                        className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 resize-none max-h-[200px] transition-all"
                                        rows={3}
                                        maxLength={2000}
                                    />
                                ...
                                <button
                                    ...
                                    className={cn(
                                        "px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-xl font-semibold transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed",
                                ...
                                >
                                    {isPosting ? (
                                        ...
                                        <>
                                            <IconSend size={18} />
                                            <span>{t.leaderboard.post_comment}</span>
                                        </>
                                    )}
                                </button>
                            </div>
